...
/**
 * REST controller for managing {@link com.lillah.domain.Person}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PersonResource {
    ...
    @GetMapping("/people/cnic/{cnic}")
    public ResponseEntity<Person> getPersonByCNIC(@PathVariable String cnic) {
        log.debug("REST request to get by cnic : {}", cnic);
        Optional<Person> person = Optional.empty();
        try {
            List<Person> personList = personRepository.findByCnic(cnic);
            if (personList != null && personList.size() > 0)
                person = Optional.ofNullable(personList.get(0));

        } catch (Exception ex) {
            log.error("ERROR in getPersonByCNIC: " + ex.toString());
        }
